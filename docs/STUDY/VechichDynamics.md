...
![2_1_1](image/VehichDyanmics_2_1_1.png)
...
好的路面需要弹簧刚度小一些，坏路面要求弹簧刚度要大一些，因此需要车辆
在动行程大的时候，刚度大一些，动行程小的时候刚度小一些

![2_2_1](image/VehicleDynamic_2_2_1.png)
...
可以根据气室数量分为：单气室油气弹簧、双气室油气弹簧、具有反压气室的油气弹簧、可调式油气弹簧

## 车辆动力传动系统动力学

* 动力部件：发动机、电机
* 传动部件：机械变速箱、液力机械自动变速箱、双离合器变速箱、机电复合传动

* **系统特点**
* 多个档位或多种工作模式，运行工况复杂
* 多源强激励，多种非线性因素，动力学特性复杂

### 一、系统分类

* 线性系统:系统元件均是线性元件或元素
* 非线性系统:系统含有至少一个非线性元件或元素
* 定常系统：所有模型参数都不随时间变化
* 时变系统：一个或一个以上模型参数随时间变化
* 集总参数系统：组成系统的元件皆为集总参数元件
* 分布参数系统：系统中分布参数元件

### 二、模型分类

* 机理模型：基于牛顿第二定律或者拉格朗日方程等建立数学模型
* 仿真模型：通过软件可以求解机理模型得到的微分方程，进行建模和仿真分析

### 三、常用的数学模型

* 微分方程：不能得到解析解，不能获取中间的量
* 传递函数：
=======
## 第三章 转向轮和前轴的横摆
